Shader "Frosty/MixedLighting"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 shiftHue(float4 originalColor, float hue) {
			...
			float4 replaceColor(float4 originalColor, float4 source, float4 target, float tolerance) {
				float4 col = originalColor;
				float colorDist = distance(getHue(col), getHue(source));
				
				if (colorDist < tolerance / 255) {
					float a = getHue(col) * 360 - getHue(target) * 360;
					col = shiftHue(col, a);
				}
				return col;
			}
